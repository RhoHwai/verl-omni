#!/usr/bin/env bash
# FLUX.1-dev full-parameter DanceGRPO recipe, aligned with v2_2.sh.
set -euo pipefail
...
: "${TRAIN_FILES_PATH:?Set TRAIN_FILES_PATH to train.parquet}"
: "${VAL_FILES_PATH:?Set VAL_FILES_PATH to test.parquet}"
: "${CUSTOM_REWARD_MODEL_PATH:?Set CUSTOM_REWARD_MODEL_PATH to the selected reward checkpoint}"

REWARD_NAME=${REWARD_NAME:-hpsv2}
case "$REWARD_NAME" in
    hpsv2)
        ...
        export HPSV2_PRETRAINED_PATH REWARD_DEVICE
        ;;
    hpsv3)
        ...
        ;;
    *)
        echo "Error: REWARD_NAME must be hpsv2 or hpsv3, got '$REWARD_NAME'." >&2
        exit 1
        ;;
esac

export CUSTOM_REWARD_MODEL_PATH
...
python3 -m verl_omni.trainer.main_diffusion \
    ...
    actor_rollout_ref.rollout.algo.timestep_sample_strategy=random_subset \
    actor_rollout_ref.rollout.algo.timestep_fraction=0.6 \
    actor_rollout_ref.rollout.algo.drop_last_transition=true \
