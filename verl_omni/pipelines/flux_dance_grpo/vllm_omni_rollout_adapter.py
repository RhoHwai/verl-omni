...
class FluxDanceGRPOPipelineWithLogProb(FluxPipeline):
    ...
    def _set_timesteps(self, *, num_steps: int, shift: float) -> torch.Tensor:
        ...
        shifted = sd3_time_shift(shift, sigmas)
    ...
    def forward(
        ...
        **kwargs: Any,
        ...
        strategy = _coalesce_not_none(
            extra_args.get("timestep_sample_strategy"), kwargs.get("timestep_sample_strategy", "continuous")
        )
        fraction = float(_coalesce_not_none(extra_args.get("timestep_fraction"), kwargs.get("timestep_fraction", 1.0)))
        drop_last = bool(
            _coalesce_not_none(extra_args.get("drop_last_transition"), kwargs.get("drop_last_transition", False))
        )
